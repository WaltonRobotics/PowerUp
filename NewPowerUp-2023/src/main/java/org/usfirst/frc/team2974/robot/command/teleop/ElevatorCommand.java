...
public class ElevatorCommand extends CommandBase {
	...
	public void execute() {
		if (elevator.isMotionControlled()) {
			if (elevatorNudgeUp.getAsBoolean() && !elevator.atTopPosition()) {
				elevator.nudge(NUDGE_DISTANCE);
			}

			if (elevatorNudgeDown.getAsBoolean() && !elevator.atLowerPosition()) {
				elevator.nudge(-NUDGE_DISTANCE);
			}

			if (elevatorHigh.getAsBoolean()) {
				elevator.setTarget(HIGH_HEIGHT);
			} else if (elevatorMedium.getAsBoolean()) {
				elevator.setTarget(MEDIUM_HEIGHT);
			} else if (elevatorLow.getAsBoolean()) {
...
//		if (elevatorToggleControl.getAsBoolean()) {
//			if (elevator.isMotionControlled()) {
//				elevator.disableControl();
//			} else {
